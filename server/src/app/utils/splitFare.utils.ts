...
export const getSurchargeMultiplier = (totalSeats: number): { multiplier: number; percent: number } => {
  if (totalSeats >= 6) return { multiplier: 1.40, percent: 40 };
  if (totalSeats >= 5) return { multiplier: 1.20, percent: 20 };
  return { multiplier: 1.00, percent: 0 };
};
...
export const calcSplitPassengerFare = async (
  ...
  const holCharge  = isHoliday ? s.holidayIncreasePercentage : 0;

  const { multiplier, percent } = getSurchargeMultiplier(totalSeats);

  const basePerSeat = (initial + kmCharge + lugCharge + holCharge) / totalSeats * requestedSeats;
  const surchargeAmount = Math.round(basePerSeat * (multiplier - 1) * 100) / 100;
  const estimatedFare   = Math.round(basePerSeat * multiplier * 100) / 100;

  return {
    initialCharge:    Math.round(initial / totalSeats * requestedSeats * 100) / 100,
    totalKmCharge:    Math.round(kmCharge / totalSeats * requestedSeats * 100) / 100,
    luggageCharge:    Math.round(lugCharge / totalSeats * requestedSeats * 100) / 100,
    holidayTripCharge: Math.round(holCharge / totalSeats * requestedSeats * 100) / 100,
...
export const acquireRecalculateLock = async (rideId: string, ttl = 15): Promise<boolean> => {
  const redis  = getRedisClient();
  // Use argument-style flags for older redis client typings: EX <seconds> NX
  const result = await redis.set(`ride:recalculate:lock:${rideId}`, '1', 'EX', ttl, 'NX');
  return result === 'OK';
};
...
export const chargeUser = async (
  ...
  if (!user) return { success: false, method: 'none', failReason: 'user_not_found' };
  ...
  if (wallet >= rounded) {
    ...
    io?.to(`user:${userId}`).emit('ride:payment-charged', { amount: rounded, method: 'wallet', reason });
  ...
  if (user.customerId && cardPortion > 0) {
    try {
      ...
      io?.to(`user:${userId}`).emit('ride:payment-charged', { amount: rounded, method, reason });
  ...
  io?.to(`user:${userId}`).emit('ride:payment-failed', {
    amount: rounded, reason, message: 'No payment method found.',
  });
...
export const recalculateSplitFares = async (
  ...
  try {
    ...
    const totalSeats = activePassengers.reduce((s, p) => s + (p.requestedSeats || 1), 0);
    const { percent: newSurchargePercent } = getSurchargeMultiplier(totalSeats);

    const ride = await Ride.findById(rideId).lean();
    if (!ride) return;

    const depDate = new Date(`${(ride as any).departureDate}T${(ride as any).departureTime}:00`);
    ...
    for (const passenger of activePassengers) {
      ...
      if (diff < 0) {
        ...
        const result = await chargeUser(
          passenger.userId.toString(), diff, rideId, `fare_recalculate_${reason}`, io,
        );
        if (!result.success) {
          // Case 11 — flag for manual recovery
          await Passenger.findByIdAndUpdate(passenger._id, { paymentStatus: 'pending_recovery' });
        }
      }
    ...
    await Ride.findByIdAndUpdate(rideId, { currentSurchargePercent: newSurchargePercent });
    console.log(`✅ Recalculated | ride: ${rideId} | reason: ${reason} | seats: ${totalSeats} | surcharge: ${newSurchargePercent}%`);
  } finally {
...
export const calculateCancellationRefund = async (
  ...
  bookingTime:   Date,
  departureTime: Date,
): Promise<{ refundAmount: number; platformAmount: number; refundReason: string }> => {
  const settings = await Setting.find({
    key: { $in: ['cancellationFreeWindowHours', 'cancellationPercentage50Hours'] },
  }).lean();
  ...
  if (hoursSinceBook <= freeWindowHours)
    return { refundAmount: paidAmount, platformAmount: 0, refundReason: 'free_window' };

  // 50% penalty
  if (hoursBeforeDep <= penalty50Hours) {
    const refund = Math.round(paidAmount * 0.5 * 100) / 100;
    return { refundAmount: refund, platformAmount: paidAmount - refund, refundReason: 'penalty_50' };
  }

  // Outside penalty window — full refund
  return { refundAmount: paidAmount, platformAmount: 0, refundReason: 'outside_penalty_window' };
};
