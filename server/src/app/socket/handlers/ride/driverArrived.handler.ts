...
const checkDriverNearPickup = async (
  ...
  return { isNear: distanceMeters <= ARRIVAL_THRESHOLD_METERS, distanceMeters: Math.round(distanceMeters) };
};
  ...
  async (socket: TSocket, data: any, callback?: any) => {
    ...
    const notifyPassenger = async (passenger: any, isLastArrival = false) => {
      ...
      await redis.rpush(`ride:${rideId}:live`, JSON.stringify({
        driverId, event: 'ARRIVED_AT_PICKUP',
        passengerId: passenger._id, lat, lng, timestamp: Date.now(),
      }));

      // Socket
      io.to(`user:${passenger.userId}`).emit('ride:driver-arrived', {
        rideId, passengerId: passenger._id, driverId,
        message: 'Driver has arrived at your pickup location',
        waitingTime: 2, isLastArrival,
      });
    ...
    if (ride.type === RIDE_TYPE.private) {
      const passenger = await Passenger.findOne({ rideId, status: PASSENGER_STATUS.confirmed });
      if (!passenger)
        return callback?.({ success: false, message: 'No active passenger found' });

      const { isNear, distanceMeters } = await checkDriverNearPickup(
        redis, driverId, passenger.pickup.coordinates[1], passenger.pickup.coordinates[0], lat, lng,
      );
      ...
      if (!isNear)
        return callback?.({
          ...
          distanceMeters, thresholdMeters: ARRIVAL_THRESHOLD_METERS,
        });

      await notifyPassenger(passenger, true);

      return callback?.({
        success: true, message: 'Driver arrived notification sent',
    ...
    if (passengerId && !arriveAll) {
      const passenger = await Passenger.findOne({ _id: passengerId, rideId, status: PASSENGER_STATUS.confirmed });
      if (!passenger)
    ...
    if (arriveAll) {
      const passengers = await Passenger.find({ rideId, status: PASSENGER_STATUS.confirmed, arrivedNotified: false });
      ...
      for (let i = 0; i < passengers.length; i++) {
        ...
        if (!isNear) {
          results.push({ passengerId: passenger._id, notified: false, distanceMeters, message: `Not near pickup (${distanceMeters}m away)` });
          continue;
        }
