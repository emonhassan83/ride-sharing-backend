...
export const rideCancelAfterAcceptHandler = eventHandler<any>(
  async (socket: TSocket, data: any, callback?: any) => {
    ...
    // ── Cancellation refund calculation (Cases 20, 21) ────────────────────────
    ...
    // ── Refund to wallet (Case 26 — always wallet) ────────────────────────────
    if (refundAmount > 0) {
    ...
    if (ride.driverId) {
      await redis.hincrby(`driver:${ride.driverId}:details`, 'bookedSeats', -(passenger.requestedSeats || 1));
    }
    ...
    // ── SPLIT RIDE ────────────────────────────────────────────────────────────
    // Case 3: rideCreatedBy cancelled?
    if (ride.rideCreatedBy?.toString() === userId) {
      ...
      if (!transferred) {
        ...
        await Ride.findByIdAndUpdate(rideId, {
          status: RIDE_STATUS.cancelled, cancellationReason: 'Creator cancelled',
    ...
    const remainingCount = await Passenger.countDocuments({
      rideId, status: { $nin: ['cancelled', 'rejected'] },
    });

    if (remainingCount === 0) {
      await Ride.findByIdAndUpdate(rideId, {
        status: RIDE_STATUS.cancelled, cancellationReason: 'Last passenger cancelled',
        cancelledBy: CANCELLED_BY.user, cancelledAt: new Date(),
      });
      await Promise.all([
        redis.del(`ride:active:${rideId}`),
    ...
    // Recalculate remaining passengers (Cases 6, 29, 30)
    await recalculateSplitFares(rideId, 'passenger_cancelled', io);

    // Notify driver + co-passengers
    if (ride.driverId) {
      io.to(`driver:${ride.driverId}`).emit('ride:passenger-cancelled', {
        rideId, passengerId: passenger._id, remainingPassengers: remainingCount,
        message: 'A passenger cancelled.',
      });
    }

    const others = await Passenger.find({
      rideId, _id: { $ne: passenger._id }, status: { $nin: ['cancelled', 'rejected'] },
    }).select('userId');

    for (const p of others) {
      io.to(`user:${p.userId}`).emit('ride:co-passenger-cancelled', {
        rideId, cancelledPassengerId: passenger._id,
    ...
    return callback?.({
      ...
      data:    { refundAmount, refundReason, rideCancelled: false, remainingPassengers: remainingCount },
    });
  },
);