// handlers/ride/splitRideRequest.handler.ts — Bug 8 fix
...
import { calcSplitPassengerFare } from '../../../utils/splitFare.utils'; // ✅ Bug 8 fix
...
function isPointNearRoute(lat: number, lng: number, routeCoordinates: number[][]): boolean {
  ...
  for (let i = 0; i < routeCoordinates.length - 1; i++) {
    ...
    const dist = haversineMeters(lat, lng, lat1 + t * (lat2 - lat1), lng1 + t * (lng2 - lng1));
...
export const joinSplitRideRequestHandler = eventHandler<any>(
  async (socket: TSocket, data: any, callback?: any) => {
    ...
    for (const ride of matchingRides) {
      ...
      // ✅ Bug 8 fix: calcSplitPassengerFare with correct totalSeats
